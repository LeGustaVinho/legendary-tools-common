...
public static class SelectionUtil
{
    ...
    /// </summary>
    public static void HandleEmptyLeftClick(DagEditorContext ctx, Rect canvasRect, System.Func<bool> isOverAnyNode)
    {
        Event e = Event.current;
        if (e.type == EventType.MouseDown && e.button == 0 && canvasRect.Contains(e.mousePosition) && !isOverAnyNode())
        {
            if (!e.shift)
            {
    ...
    /// </summary>
    public static void HandleDeleteShortcut(DagEditorContext ctx)
    {
        Event e = Event.current;
        if (e.type == EventType.KeyDown && e.keyCode == KeyCode.Delete)
        {
            ...
            if (ctx.SelectedNodes.Count > 0)
            {
                ...
                foreach (IDagNode n in ctx.SelectedNodes.ToList())
                {
            ...
            if (ctx.SelectedEdges.Count > 0)
            {
                ...
                foreach (IDagEdge<IDagNode> edge in ctx.SelectedEdges.ToList())
                {
