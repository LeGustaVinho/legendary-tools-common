...
public class InspectorPanel
{
    ...
    /// </summary>
    public void Draw(DagEditorContext ctx, Rect inspectorRect)
    {
        ...
        if (ctx.SelectedNodes.Count > 0 && ctx.SelectedEdges.Count > 0)
        {
            EditorGUILayout.HelpBox(
                $"{ctx.SelectedNodes.Count} node(s) and {ctx.SelectedEdges.Count} edge(s) selected.", MessageType.Info);
            GUILayout.EndArea();
            return;
        }
        ...
        if (ctx.SelectedEdges.Count == 0)
        {
            ...
            {
                IDagNode node = ctx.SelectedNodes.FirstOrDefault();
                if (node is Object uo)
                {
                    if (ctx.CachedNodeEditor == null || ctx.CachedNodeEditor.target != uo)
                    {
                        ...
                        ctx.CachedNodeEditor = Editor.CreateEditor(uo);
                    }
        ...
        if (ctx.SelectedNodes.Count == 0)
        {
            ...
            {
                IDagEdge<IDagNode> edge = ctx.SelectedEdges.FirstOrDefault();
                if (edge is Object eo)
                {
                    if (ctx.CachedEdgeEditor == null || ctx.CachedEdgeEditor.target != eo)
                    {
                        ...
                        ctx.CachedEdgeEditor = Editor.CreateEditor(eo);
                    }
