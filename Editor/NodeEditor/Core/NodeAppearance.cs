...
public class NodeAppearance
{
    ...
    /// </summary>
    public Rect GetNodeRect(IDagNode node, Dictionary<Type, FieldInfo[]> showCache)
    {
        ...
        float dynamicAdd = fields.Length > 0
            ? fields.Length * (DagEditorLayout.InlineFieldHeight + 4f) + 6f
            : 0f;

        float height = DagEditorLayout.DefaultNodeBaseHeight + dynamicAdd;
        return new Rect(node.Position, new Vector2(DagEditorLayout.DefaultNodeWidth, height));
    }
    ...
    /// </summary>
    public GUIStyle ResolveStyle(DagEditorContext ctx, IDagNode node, bool selected)
    {
        if (node == null)
    ...
    /// </summary>
    public void DrawInlineFields(IDagNode node, Dictionary<Type, FieldInfo[]> cache)
    {
    ...
    private static FieldInfo[] GetShowInNodeFields(IDagNode node, Dictionary<Type, FieldInfo[]> cache)
    {
