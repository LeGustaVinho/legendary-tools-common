...
namespace LegendaryTools.NodeEditor
{
    ...
    public class ResizeController
    {
        ...
        /// </summary>
        public void HandleResize(NodeEditorContext ctx, Rect canvasRect, System.Func<IEditorNode, Rect> getNodeRect)
        {
            ...
            foreach (IEditorNode n in ctx.Graph.Nodes)
            {
                if (n == null) continue;
            ...
            if (e.type == EventType.MouseDown && e.button == 0 && !ctx.Resize.Active)
                ...
                {
                    IEditorNode n = ctx.Graph.Nodes[i];
                    if (n == null) continue;
            ...
            if (e.type == EventType.MouseDrag && e.button == 0 && ctx.Resize.Active && ctx.Resize.Node != null)
            {
                ...
                newW = Mathf.Max(NodeEditorLayout.MinNodeWidth, newW);
                newH = Mathf.Max(NodeEditorLayout.MinNodeHeight, newH);
        ...
        private static void AddResizeCursors(Rect logicalRect, Rect canvasRect, NodeEditorContext ctx)
        {
            float band = NodeEditorLayout.ResizeBandPx;
        ...
        private static void GetSidesUnderMouse(Rect logicalRect, Rect canvasRect, NodeEditorContext ctx,
        ...
        private static void ApplyResize(NodeEditorContext ctx, IEditorNode node, float x, float y, float w, float h)
        {
