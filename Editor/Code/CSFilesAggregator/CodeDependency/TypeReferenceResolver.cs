// Assets/legendary-tools-common/Editor/Code/CSFilesAggregator/DependencyScan/TypeReferenceResolver.cs
using System;
...
namespace LegendaryTools.CSFilesAggregator.DependencyScan
{
    ...
    internal sealed class TypeReferenceResolver
    {
            ...
            out IReadOnlyList<TypeIndexEntry> entries)
        {
            ...
            if (typeIndex == null || candidate == null)
            {
                return false;
            }
            ...
            {
                if (context.UsingAliases.TryGetValue(name, out string aliasTarget) && !string.IsNullOrEmpty(aliasTarget))
                {
                    ...
                    if (typeIndex.TryGet(normalizedAliasTarget, out entries))
                    {
                        return true;
                    }
            ...
            if (candidate.IsQualified)
            {
                ...
                if (typeIndex.TryGet(normalizedQualified, out entries))
                {
                    return true;
                }
            ...
            if (context != null)
            {
                if (!string.IsNullOrEmpty(context.Namespace))
                {
                    ...
                    if (typeIndex.TryGet(fq, out entries))
                    {
                        return true;
                    }
                }
            ...
            if (typeIndex.TryGet(name, out entries))
            {
                return true;
            }
