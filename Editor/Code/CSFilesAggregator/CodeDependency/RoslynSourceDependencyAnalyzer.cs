...
namespace LegendaryTools.CSFilesAggregator.DependencyScan
{
    ...
    internal sealed class RoslynSourceDependencyAnalyzer
    {
            ...
            List<TypeReferenceCandidate> result)
        {
            ...
            if (cus != null) ExtractUsingsAndAliases(cus, context);
        ...
        private static void ExtractUsingsAndAliases(CompilationUnitSyntax root, SourceFileContext context)
        {
            foreach (UsingDirectiveSyntax u in root.Usings)
            {
                ...
                if (u.Alias != null && !string.IsNullOrEmpty(u.Alias.Name?.Identifier.ValueText))
                {
                    ...
                    if (!context.UsingAliases.ContainsKey(alias)) context.UsingAliases.Add(alias, name);

                    continue;
                }

                if (!context.Usings.Contains(name)) context.Usings.Add(name);
            }
        }
        ...
        private static string GetDeclaredNamespace(SyntaxNode root)
        {
            ...
            foreach (SyntaxNode node in root.DescendantNodes(descendIntoTrivia: false))
            {
                if (node is NamespaceDeclarationSyntax nds)
                {
                    ...
                    if (!string.IsNullOrEmpty(name)) parts.Push(name);
                }
        ...
        private static void AddCandidateFromTypeSyntax(SyntaxTree tree, TypeSyntax typeSyntax,
            List<TypeReferenceCandidate> result)
        {
        ...
        private static void AddCandidateFromNameSyntax(SyntaxTree tree, NameSyntax nameSyntax,
            List<TypeReferenceCandidate> result)
        {
        ...
        private static string NormalizeTypeSyntax(TypeSyntax typeSyntax)
        {
            ...
            if (typeSyntax is PredefinedTypeSyntax) return null;

            if (typeSyntax is IdentifierNameSyntax ins) return ins.Identifier.ValueText;
            ...
            if (typeSyntax is QualifiedNameSyntax qns) return NormalizeQualifiedName(qns);
        ...
        {
            if (nameSyntax is IdentifierNameSyntax ins) return ins.Identifier.ValueText;
            ...
            if (nameSyntax is QualifiedNameSyntax qns) return NormalizeQualifiedName(qns);

            if (nameSyntax is AliasQualifiedNameSyntax aqns) return NormalizeNameSyntax(aqns.Name);
